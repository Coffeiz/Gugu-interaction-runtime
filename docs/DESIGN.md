...
> 当前实现版本：3.0.6。公开接口以 [API 参考](./API.md) 为准，接入步骤见
> [接入指南](./INTEGRATION.md)。
...
对于列表卡片这类需要“抓起时收紧”的场景，业务可以在对象类型上配置
`proxyLayout.compact`。它只描述代理的目标宽度、定位和过渡参数，Runtime 负责
首帧尺寸、抓起后的连续变换以及 landing 时恢复本体尺寸；文件名、图标和信息列的
排列仍由业务卡片自己的 Grid/Flex 样式负责。这样不会把某一种列表布局硬编码进
Runtime，也不会让每个业务页面重复实现代理尺寸动画。
