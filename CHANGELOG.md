...
- **画布自由落地稳定性提升**：完善普通/物理落地、速度限制、回正节奏，以及跨画布、缩放、平移、regrab 和落地尺寸变化时的目标同步；补充对应回归覆盖。
- **抽屉与 Surface 布局稳定性提升**：修复抽屉高度测量、组 FLIP、滚动自动定位、收起间距、底部占位和折叠状态落点限制等布局事务问题。
- **节点与连接线交互收口**：补齐画布节点注册、连接点、连接线跟随卡片移动，以及附加交互的可见性和生命周期管理。
- **Landing 视觉交接稳定性提升**：修复双快照交接、首帧跳变、跨 Surface 尺寸回退、阴影继承和透视倾角等问题，统一代理生命周期清理。
