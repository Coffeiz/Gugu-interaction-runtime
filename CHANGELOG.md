...
### 修复
- `preserveMoveTarget` 改为释放时就地采样：业务 Action 的乐观更新可能在 landing 期间卸载被拖对象（如文件移出当前目录后卡片组件卸载），此前 landing 中途查不到对象会误判为 false，把语义落点（面包屑/文件夹）conceal 隐藏，表现为拖入动画中面包屑消失。
- regrab 的落点目标排除判断改为实时解析语义 target：业务重渲染替换目标节点后，旧元素引用失配会导致点击仍被 regrab 抢走。

## v3.0.5

### 修复
- 落地动画期间按在落点目标（面包屑/文件夹等语义 target）上不再触发 regrab：此前飞行代理正好盖在目标上方，按代理矩形命中会把点击目标的 pointerdown 抢走，导致动画结束前点击目标目录无法进入。

## v3.0.4

### 修复
- 恢复移动视觉事务的 `move-visual-settled` 收尾事件，区分视觉跟踪结束与完整事务收尾。

## v3.0.3

### 文档
- 重排接入文档章节，新增 Core API 与视觉策略参考，并归档历史实施计划。
- 增加看板、文件库、画布自由落点和跨容器拖拽案例展示。
- 完成 npm 公开发布元数据整理，统一许可证和仓库地址信息。

## v3.0.2

### 改进
- **布局测量与 FLIP 性能优化**：以布局事务统一复用几何测量上下文，减少重复的 Surface/参与者扫描，并合并 landing 期间的帧调度，降低复杂拖拽场景的主线程开销。
- **Presence 入场语义收口**：支持新的 Presence 语义条目，正确识别折叠分组中的有效载体，减少无效几何读取并补齐完成列折叠卡片的入场动画。
- **落地目标追踪稳定化**：抽屉展开、布局重排和并发 retarget 期间持续追踪 landing 目标边界，保持同一 Surface 回位和嵌套 Surface 的参与者边界稳定。

### 修复
- 修复画布矩形落点在代理开始前短暂显示的问题，并确保只有当前拖拽 session 能恢复目标可见性。
- 修复移动视觉事务收尾事件未可靠触发的问题，清理无效的画布拖拽诊断探针。

### 测试
- 补充布局测量上下文、FLIP 参与者裁剪、Presence 语义、并发 landing retarget 和目标可见性回归覆盖。

## v3.0.1

### 改进
- **Landing 视觉交接连续性**：抓取代理运动归属统一收口，修复交接瞬间跳跃与 hover 快照污染，优化落地视觉快照几何读取。
- **项目卡落地视觉过渡平滑化**：落地过程视觉过渡平滑，避免落地瞬间样式跳变。
- **拖拽代理层级可配置**：支持业务侧配置对象拖拽代理层级，并收敛落地代理层级参数类型。
- **浮动抽屉布局稳定性**：缓存浮动抽屉同帧尺寸测量，修复嵌套分组高度缓存与收起动画、分组测量导致滚动位置归零，以及分组收起时的 Surface 缓存复用。
